...
contract Vesting is Ownable {
    ...
    event VestDetails(address account, uint256 totalAmount);

    constructor() {}

    function getNumOfSchedules(address sender)public view returns(uint){
        return numberOfSchedules[sender];
    }

    function getScheduleTotalAmount(uint scheduleNumber)public view returns(uint){
        Schedule memory schedule = schedules[msg.sender][scheduleNumber];
        return schedule.totalAmount;

    }

    function setApprovedAddress(address approveAddress)public onlyOwner(){
        isApproved[approveAddress] = true;
    }

    modifier Authorized() {
        require(owner() == _msgSender() || isApproved[_msgSender()]  , "Ownable: caller is not the owner");
        _;
    }
    ...
    function claim(uint256 scheduleNumber) external {
        ...
        emit VestDetails(msg.sender, schedule.totalAmount);
        require(
